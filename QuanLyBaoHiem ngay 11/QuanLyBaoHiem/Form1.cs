...
namespace QuanLyBaoHiem
{
    public partial class frmMain : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        public void loadghinhothongbao()
        {
            string path = "C:\\temp\\thongbao.ini";
            if (File.Exists(path) == true)
            {
                string[] lines = File.ReadAllLines(@"C:\temp\thongbao.ini");
                int co = 0;
                for (int i = 0; i < lines.Count(); i+=2)
                {
                    if (lines[i] == taikhoanhientai && lines[i+1]=="1")
                    {
                        co = 1;
                        break;
                    }
                ...
                if (co==0)
                {
                    //Luunguoidangnhap1lan.Manv = taikhoanhientai;
                    update();
                    ucThongBaoSinhNhat uc = new ucThongBaoSinhNhat();
                    ucThongBaoDongHoaDon uc1 = new ucThongBaoDongHoaDon();
                }
        ...
        public void update()
        {
            String filepath = "C:\\temp\\thongbao.ini";
            string[] lines = File.ReadAllLines(@"C:\temp\thongbao.ini");
            FileStream fs = new FileStream(filepath, FileMode.Create);//chỉnh file 
            StreamWriter sWriter = new StreamWriter(fs, Encoding.UTF8);//fs là 1 FileStream 

            for (int i = 0; i < lines.Count(); i += 2)
            {
                if(lines[i]==taikhoanhientai)
                {
                    lines[i + 1] = "1";
                }
            }
